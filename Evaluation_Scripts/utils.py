...
def data_loading(path_to_df: os.PathLike):
...
def pruned_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for i in range(len(df)):
        if df.loc[i]["information.language"] in top_4_lang:
            sub_df.loc[idx] = [
                df.loc[i]["information.sessionId"],
                df.loc[i]["information.language"],
                None,
            ]
            idx += 1
...
def download_recordings(
    df: pd.DataFrame,
    audio_save_dir: os.PathLike,
    utterances_save_dir: os.PathLike,
    clipped_audio_dir: os.PathLike,
):
    ...
    for i in files_list:
        cmd = (
            ...
            + i
            + '" -map_channel 0.0.0 "'
            + os.path.join(utterances_save_dir, i.split("/")[-1].split(".mp3")[0])
            + '_customer.wav"'
        )
        runcmd(cmd)
    ...
    for i in utterance_list:
        ...
        for chunk in audio_chunks:
            combined += chunk
        combined.export(
            "{0}/{1}".format(os.path.abspath(clipped_audio_dir), i.split("/")[-1]),
            format="wav",
        )
...
def make_predictions(model_id, utter_dir, sub_df):
    ...
    print(
        "Time taken for evaluation of {} samples is {:.2f} seconds.\nLatency is: {:.2f}".format(
            total, t2 - t1, ((t2 - t1) / total)
        )
    )
    ...
    if model_id:
        runcmd(
            "rm *.wav"
        )  # Model creates copies of wav files in the pwd which must be deleted

    # Convert Languages to IDs
    label_to_Int = (
        {"English": 0, "Hindi": 1, "Tamil": 3, "Kannada": 3}
        if model_id
        else {"English": 0, "Hindi": 1, "Tamil": 2, "Kannada": 3}
    )  # make changes if newer models are added
    for i in range(len(sub_df)):
...
def evaluate(model_id):
    ...
    try:
        print(
            "Percent unrecognized: {:.2f}".format(
                res_count_dict[4] / sum(res_count_dict.values()) * 100
            )
        )
    ...
    print(
        "Accuracy including not identified: {:.2f}".format(
            correct_not_det / len(y) * 100
        )
    )
    ...
    try:
        print(
            "Accuracy excluding not identified: {:.2f}".format(
                correct_det / len(sub_df_det) * 100
            )
        )
    except ZeroDivisionError as e:
    ...
    if model_id:
        ...
        cm_df = pd.DataFrame(
            cm,
            index=["English", "Hindi", "Other", "None"],
        ...
        cm_df = pd.DataFrame(
            cm,
            index=["English", "Hindi", "Tamil", "Kannada", "None"],
